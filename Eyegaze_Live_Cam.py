...
if not cap.isOpened():
    print("Unable to read webcam")

frame_width = int(cap.get(3))
frame_height = int(cap.get(4))

out = cv2.VideoWriter('Output_Emotion_Live_Cam.mp4', cv2.VideoWriter_fourcc("m", "p", "4", "v"), 20, (frame_width, frame_height))
...
while True:
    ...
    if ret:
        ...
        cv2.putText(frame, "On Screen Percentage: " + str(onscreenpercent) + " %", (50, 530), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 255, 0), 2)
        cv2.putText(frame, "Off Screen Percentage: " + str(offscreenpercent) + " %", (50, 560), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 255, 0), 2)
        cv2.putText(frame, "Undetected Eyes Percentage: " + str(abspercent) + " %", (50, 590), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 255, 0), 2)
        cv2.putText(frame, "Number of Focused Eyes: " + str(ongaze), (50, 620), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 255, 0), 2)
        cv2.putText(frame, "Number of Unfocused Eyes: " + str(offgaze), (50, 650), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 255, 0), 2)
        cv2.putText(frame, "Number of Undetected Eyes: " + str(absgaze), (50, 680), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 255, 0), 2)

        cv2.imshow("EyeGaze On Screen-Off Screen Detection", frame)

        f = open("Result_Eyegaze_Live_Cam.txt", "w")
